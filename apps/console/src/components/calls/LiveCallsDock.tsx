...
function LiveTranscriptDrawer({
  ...
  const [whispers, setWhispers] = useState<Array<{ id: string; text: string; time: string }>>([]);
  ...
  function handleSendWhisper() {
    ...
    setWhispers((p) => [...p, newWhisper]);
    setWhisperText("");
    toast.success("Whisper coaching injected into agent context.");
  }
    ...
    <>
      <Sheet open={call !== null} onOpenChange={(open) => !open && onClose()}>
        ...
        >
          {call ? (
            <>
              <SheetHeader className="gap-0 border-b border-[#B8B8BE] bg-[#F7F7F8] p-0 text-left">
                <div className="flex items-start justify-between gap-4 p-5">
                  <div className="min-w-0">
                    <div className="mb-2 flex items-center gap-2">
                      <span className="flex size-2 rounded-full bg-emerald-500" />
                      <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#77777F]">
                ...
                {/* Live Audio Listen In Bar */}
                <div className="flex items-center justify-between border-t border-[#D9D9DD] bg-[#FFFFFF] px-5 py-2.5">
                  <div className="flex items-center gap-2">
                    <Headphones className="size-4 text-[#002FA7]" />
                    <span className="text-xs font-semibold text-[#111111]">Live Audio:</span>
                    {isListening ? (
                      <Badge variant="outline" className="text-[10px] bg-emerald-500/10 text-emerald-600 border-emerald-500/20 animate-pulse">
                        ● Streaming Audio Live
                      </Badge>
                    ) : (
                      <span className="text-xs text-[#77777F]">Inactive</span>
                    )}
                  </div>
                  <Button
                    type="button"
                    ...
                    onClick={() => {
                      ...
                      if (!isListening) {
                        toast.success("Joined LiveKit room as silent audio listener.");
                      } else {
                        toast.info("Audio stream disconnected.");
                      }
                    }}
                    className="h-7 text-xs gap-1.5 rounded-none"
                  >
                    {isListening ? <VolumeX className="size-3.5" /> : <Volume2 className="size-3.5" />}
              ...
              <div className="relative min-h-0 flex-1 bg-[#FFFFFF]">
                ...
                >
                  ...
                  {/* Render sent supervisor whispers */}
                  {whispers.map((w) => (
                    <div
                      key={w.id}
                      className="my-3 p-3 rounded-none bg-purple-500/10 border border-purple-500/20 text-xs text-purple-800 dark:text-purple-300"
                    >
                      <div className="flex items-center justify-between font-semibold text-[10px] uppercase tracking-wider mb-1">
                        <span className="flex items-center gap-1 text-purple-700 dark:text-purple-300">
                          <Sparkles className="size-3" /> Whisper Coaching (Supervisor Only)
                        </span>
              ...
              {/* Whisper Coaching Console Dock */}
              {!transcript.isEnded && (
                <div className="border-t border-[#B8B8BE] bg-[#F7F7F8] p-3">
                  <div className="flex items-center gap-2">
                    <Input
                      placeholder="Whisper guidance to agent (caller won't hear)..."
                      value={whisperText}
                      ...
                      className="h-8 text-xs bg-[#FFFFFF] rounded-none border-[#B8B8BE]"
                    />
                    <Button
                      type="button"
                      size="sm"
                      ...
                      className="h-8 text-xs gap-1.5 shrink-0 rounded-none bg-[#002FA7] text-white hover:bg-[#002785]"
                    >
                      <Send className="size-3.5" /> Whisper
                    </Button>
                  </div>
                </div>
              )}
